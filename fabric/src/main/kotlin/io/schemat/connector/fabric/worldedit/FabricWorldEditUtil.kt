...
object FabricWorldEditUtil {
    ...
    /**
     * Adapt a Fabric ServerPlayer to a WorldEdit Player via reflection.
     * Calls com.sk89q.worldedit.fabric.FabricAdapter.adaptPlayer(ServerPlayer)
     */
    private fun adaptPlayer(player: ServerPlayer): Player? {
        return try {
    ...
    fun getLocalSession(player: ServerPlayer): LocalSession? {
        return try {
    ...
    fun getClipboard(player: ServerPlayer): Clipboard? {
        return try {
    ...
    fun setClipboard(player: ServerPlayer, clipboard: Clipboard) {
