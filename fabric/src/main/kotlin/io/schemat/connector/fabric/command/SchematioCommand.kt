...
object SchematioCommand {
     ...
     */
    private fun hasOpPermission(source: CommandSourceStack): Boolean {
    ...
    fun register(dispatcher: CommandDispatcher<CommandSourceStack>, mod: SchematioConnectorMod) {
    ...
    private fun executeInfo(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source

        source.sendSystemMessage(Component.literal("\u00a76=== Schematio Connector ==="))
        source.sendSystemMessage(Component.literal("\u00a77Platform: \u00a7fFabric"))
        source.sendSystemMessage(Component.literal("\u00a77API URL: \u00a7f${mod.apiEndpoint}"))

        val connected = mod.httpUtil != null
        val status = if (connected) "\u00a7aConnected" else "\u00a7cNot connected"
        source.sendSystemMessage(Component.literal("\u00a77Status: $status"))

        if (!connected) {
            source.sendSystemMessage(Component.literal("\u00a77Run \u00a7e/schematio settoken <token>\u00a77 to connect"))
        }
    ...
    private fun executeList(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod, rawSearch: String?): Int {
        val source = ctx.source

        if (mod.httpUtil == null) {
            source.sendSystemMessage(Component.literal("\u00a7cNot connected to API. Set token first."))
            return 0
        }
        ...
        source.sendSystemMessage(Component.literal("\u00a77Fetching schematics..."))
            ...
            try {
                ...
                mod.platformAdapter.runOnMainThread {
                    if (schematics.isEmpty()) {
                        source.sendSystemMessage(Component.literal("\u00a77No schematics found."))
                    } else {
                        source.sendSystemMessage(Component.literal("\u00a76=== Schematics ==="))
                        schematics.forEach { schematic ->
                            val name = schematic.get("name")?.asString ?: "Unknown"
                            val shortId = schematic.get("short_id")?.asString ?: ""
                            source.sendSystemMessage(Component.literal("\u00a77- \u00a7f$name \u00a78($shortId)"))
                        }

                        val currentPage = meta.get("current_page")?.asInt ?: 1
                        val lastPage = meta.get("last_page")?.asInt ?: 1
                        source.sendSystemMessage(Component.literal("\u00a77Page $currentPage of $lastPage"))
                    }
                }
            } catch (e: Exception) {
                mod.platformAdapter.runOnMainThread {
                    source.sendSystemMessage(Component.literal("\u00a7cError: ${e.message}"))
                }
            }
        }
    ...
    private fun showDownloadInputDialog(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
    ...
    private fun executeDownload(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod, rawArgs: String): Int {
        val source = ctx.source
        val player = source.player
        ...
        if (!FabricWorldEditUtil.isAvailable()) {
            source.sendSystemMessage(Component.literal("\u00a7cWorldEdit is not installed. Install WorldEdit to use download."))
            return 0
        }
        ...
        if (isQuickShare) {
            ...
            if (codeValidation is ValidationResult.Invalid) {
                source.sendSystemMessage(Component.literal("\u00a7c${codeValidation.message}"))
                return 0
            }
        ...
        source.sendSystemMessage(Component.literal("\u00a77Downloading $label \u00a7f$downloadId\u00a77..."))

        mod.platformAdapter.runAsync {
            try {
                val requestBody = JsonObject().apply {
                    addProperty("format", "schem")
                    addProperty("player_uuid", player.stringUUID)
                    ...
                    mod.platformAdapter.runOnMainThread {
                        if (clipboard != null) {
                            ...
                            if (isDialog && !isQuickShare) {
                                ...
                                source.sendSystemMessage(Component.literal("\u00a7a${label.replaceFirstChar { it.uppercase() }} downloaded to clipboard!"))
                                source.sendSystemMessage(Component.literal("\u00a77Use \u00a7e//paste\u00a77 to place it."))
                            }
                        } else {
                            source.sendSystemMessage(Component.literal("\u00a7cFailed to parse schematic data."))
                        }
                    }
                } else {
                    mod.platformAdapter.runOnMainThread {
                        ...
                        source.sendSystemMessage(Component.literal("\u00a7c$errorMsg"))
                    }
                }
            } catch (e: Exception) {
                mod.platformAdapter.runOnMainThread {
    ...
    private fun executeUpload(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
        ...
        if (!FabricWorldEditUtil.isAvailable()) {
            source.sendSystemMessage(Component.literal("\u00a7cWorldEdit is not installed. Install WorldEdit to use upload."))
            return 0
        }

        val clipboard = FabricWorldEditUtil.getClipboard(player)
        if (clipboard == null) {
            source.sendSystemMessage(Component.literal("\u00a7cNo clipboard found. Use \u00a7e//copy\u00a7c first."))
            return 0
        }

        val schematicBytes = FabricWorldEditUtil.clipboardToByteArray(clipboard)
        if (schematicBytes == null) {
            source.sendSystemMessage(Component.literal("\u00a7cFailed to convert clipboard to schematic format."))
            return 0
        }

        source.sendSystemMessage(Component.literal("\u00a77Uploading schematic (${schematicBytes.size} bytes)..."))

        mod.platformAdapter.runAsync {
            try {
                val multipart = MultipartEntityBuilder.create()
                    .addTextBody("author", player.stringUUID)
                ...
                mod.platformAdapter.runOnMainThread {
                    if (response == null) {
                        source.sendSystemMessage(Component.literal("\u00a7cCould not connect to schemat.io API."))
                        return@runOnMainThread
                    }
                    ...
                    try {
                        ...
                        if (link != null) {
                            source.sendSystemMessage(Component.literal("\u00a7aSchematic uploaded successfully!"))
                            source.sendSystemMessage(Component.literal("\u00a77Link: \u00a7f$link"))
                        } else {
                            val error = json.get("message")?.asString ?: json.get("error")?.asString ?: "Unknown error"
                            source.sendSystemMessage(Component.literal("\u00a7cUpload failed: $error"))
                        }
                    } catch (e: Exception) {
                        source.sendSystemMessage(Component.literal("\u00a7cError processing server response."))
                    }
                }
            } catch (e: Exception) {
                mod.platformAdapter.runOnMainThread {
    ...
    private fun executeQuickShare(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
        ...
        if (!FabricWorldEditUtil.isAvailable()) {
            source.sendSystemMessage(Component.literal("\u00a7cWorldEdit is not installed. Install WorldEdit to use quickshare."))
            return 0
        }
    ...
    private fun executeQuickShareWithArgs(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod, rawArgs: String): Int {
        val source = ctx.source
        val player = source.player
        ...
        if (!FabricWorldEditUtil.isAvailable()) {
            source.sendSystemMessage(Component.literal("\u00a7cWorldEdit is not installed."))
            return 0
        }
        ...
        source.sendSystemMessage(Component.literal("\u00a77Creating quick share..."))
                ...
                mod.platformAdapter.runOnMainThread {
                    if (statusCode == 201 && responseBody != null) {
                        try {
                            ...
                            if (webUrl != null) {
                                source.sendSystemMessage(Component.literal("\u00a7aQuick share created!"))
                                source.sendSystemMessage(Component.literal("\u00a77Link: \u00a7f$webUrl"))
                            } else {
                                source.sendSystemMessage(Component.literal("\u00a7eShare created but no URL returned."))
                            }
                        } catch (e: Exception) {
                            source.sendSystemMessage(Component.literal("\u00a7cError parsing response: ${e.message}"))
                        }
                    } else {
                        val errorMsg = when (statusCode) {
    ...
    private fun showSetTokenDialog(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
    ...
    private fun executeSetToken(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod, token: String): Int {
        val source = ctx.source

        mod.setApiToken(token)
        source.sendSystemMessage(Component.literal("\u00a7aAPI token updated!"))
        source.sendSystemMessage(Component.literal("\u00a77Testing connection..."))
        ...
        mod.platformAdapter.runAsync {
            try {
                ...
                mod.platformAdapter.runOnMainThread {
                    if (connected) {
                        source.sendSystemMessage(Component.literal("\u00a7aSuccessfully connected to schemat.io!"))
                    } else {
                        source.sendSystemMessage(Component.literal("\u00a7cConnection failed. Check your token."))
                    }
                }
            } catch (e: Exception) {
                mod.platformAdapter.runOnMainThread {
                    source.sendSystemMessage(Component.literal("\u00a7cConnection error: ${e.message}"))
                }
            }
        }
    ...
    private fun showSetPasswordDialog(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
    ...
    private fun executeSetPassword(
        ...
        if (password != confirm) {
            source.sendSystemMessage(Component.literal("\u00a7cPasswords do not match."))
            return 0
        }

        val validation = InputValidator.validatePassword(password)
        if (validation is ValidationResult.Invalid) {
            source.sendSystemMessage(Component.literal("\u00a7c${validation.message}"))
            return 0
        }

        source.sendSystemMessage(Component.literal("\u00a77Setting password..."))
            ...
            try {
                ...
                mod.platformAdapter.runOnMainThread {
                    if (statusCode in 200..299) {
                        source.sendSystemMessage(Component.literal("\u00a7aPassword set successfully!"))
                    } else {
    ...
    private fun showSettingsDialog(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        val player = source.player
    ...
    private fun executeSettingsSetMode(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod, mode: String): Int {
        val source = ctx.source
        source.sendSystemMessage(Component.literal("\u00a7aUI mode set to: $mode"))
        return 1
    }

    private fun executeSettingsReset(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source
        source.sendSystemMessage(Component.literal("\u00a7aUI mode reset to server default."))
        return 1
    }

    // ===== Reload =====

    private fun executeReload(ctx: CommandContext<CommandSourceStack>, mod: SchematioConnectorMod): Int {
        val source = ctx.source

        mod.reload()
        source.sendSystemMessage(Component.literal("\u00a7aConfiguration reloaded!"))
