...
/**
 * Safely get a nullable double from a JSON object.
 *
 * Unlike the defaulting getters, this distinguishes "absent/null/wrong type" (null)
 * from a real numeric value - needed for optional numeric bounds like
 * `min_value`/`max_value` on tag filter definitions.
 *
 * @param key The key to look up
 * @return The double value, or null if missing, JSON null, or not a number
 */
fun JsonObject?.safeGetDoubleOrNull(key: String): Double? {
