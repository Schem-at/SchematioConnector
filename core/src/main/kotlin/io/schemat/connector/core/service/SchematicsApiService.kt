package io.schemat.connector.core.service
 ...
 * - Error handling
 *
 * @property httpUtil The HTTP client for API requests (may be null if not configured)
 * @property platform The platform adapter for scheduling
 * @property cache The schematic cache instance
 * @property offlineMode The offline mode manager
 * @property rateLimiter The rate limiter instance
 */
class SchematicsApiService(
    private val httpUtil: HttpUtil?,
    private val platform: PlatformAdapter,
    private val cache: SchematicCache,
    ...
    fun fetchSchematicsWithCache(
        ...
        val rateLimitResult = rateLimiter.tryAcquire(playerId)
        if (rateLimitResult == null) {
            onResult(FetchResult.RateLimited(rateLimiter.getWaitTimeSeconds(playerId)))
            return
        }
    ...
    suspend fun fetchSchematicsWithOptions(options: QueryOptions): Pair<List<JsonObject>, JsonObject> {
        ...
        val http = httpUtil ?: throw Exception("API not connected - no token configured")
        val queryString = queryParams.entries.joinToString("&") { "${it.key}=${it.value}" }
        val response = http.sendGetRequest("$SCHEMATICS_ENDPOINT?$queryString")
